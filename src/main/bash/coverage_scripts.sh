...
for e0 in $e0s
for ed in $eds
qsub -q low -P cse -N $fold$e0$ed$model$optim -e logs/$model$fold$optim$e0$ed.err -o logs/$model$fold$optim$e0$ed.out -lselect=1:ncpus=4:mem=30g -lwalltime=14:00:00 -v optim=$optim,e0=$e0,ed=$ed,fold=$fold -V ./hpc_scripts/$model.sh
