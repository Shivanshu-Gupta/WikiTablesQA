...
case class ApplicationTemplate(val root: Type, val expr: Expression2,
  ...
  override def apply(state: SemanticParserState, parentInput: Expression = null): SemanticParserState = {
    ...
    val newHoles = holeIds.zip(holes).map(x => Hole(x._1, x._2._2, holeScope, x._2._3, parentInput))
...
case class LambdaTemplate(val root: Type, val args: List[Type], val body: Type) extends Template {
  ...
  override def apply(state: SemanticParserState, parentInput: Expression = null): SemanticParserState = {
    ...
    state.fill(filled, part, List(Hole(holeId, body, nextScope, false, parentInput)), this)
  }
