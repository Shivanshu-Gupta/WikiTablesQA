...
case class SemanticParserState(parts: Map[Int, ExpressionPart], unfilledHoleIds: List[Hole], nextId: Int,
  ...
  def addRootType(rootType: Type, rootParentInput: Expression): SemanticParserState = {
    ...
    SemanticParserState(parts, List(Hole(0, rootType, scope, false, rootParentInput)), 1, 0, rootType, List(), List(), List(), List(), List(), List())
  }
}
