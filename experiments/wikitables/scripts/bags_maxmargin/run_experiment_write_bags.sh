...
EXPERIMENT_TYPE="dpd_bags_maxmargin"
...
echo "Writing bags for $EXPERIMENT_TYPE"
./$SCRIPT_DIR/run.sh org.allenai.wikitables.WikiTablesSemanticParserCli --toWriteBags --bagSize 10 --numBags 10 --trainingData $TRAIN --devData $TRAIN_DEV --derivationsPath $DERIVATIONS_PATH --modelOut $MY_MODEL --epochs $EPOCHS --beamSize $BEAM_SIZE --devBeamSize $TEST_BEAM_SIZE --maxDerivations $MAX_TRAINING_DERIVATIONS --vocabThreshold $VOCAB --inputDim $INPUT_DIM --hiddenDim $HIDDEN_DIM --actionDim $ACTION_DIM --actionHiddenDim $ACTION_HIDDEN_DIM --skipActionSpaceValidation --relu --actionBias --maxPoolEntityTokenSimilarities --concatLstmForDecoder --entityLinkingMlp $@ &> $MY_DIR/train_log.txt
